...
class Evaluate(Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        loss,acc=model[1].evaluate([x_test,x_test_un],y_test)
        self.accs.append(acc)
    
        if acc >= self.highest: # 保存最优模型权重
            self.highest = acc
            model[1].save_weights('best_model.weights')
...
def build_model_dgcnn(maxlen,word_size,data_vol,batch_size):
    ...
    x3 = block_dgcnn(x3,mask3,share,0.3)
    x4 = block_dgcnn(x4,mask4,share,0.4)
 
    x=AttentionConatenate(150)([x1,x2,x3,x4])
    p = Dense(1, activation='sigmoid',name="sigmoid")(x)

    input_target = Input(shape=(1,))
    centers = Embedding(1, 1)(input_target)
    l2_loss = Lambda(lambda x: K.sum(K.square(x[0]-x[1][:,0]), 1, keepdims=True), name='l2_loss')([p,centers])

    model=Model([x_in,x_in_un,input_target],[p,l2_loss]) 
    model.compile(optimizer='adam',loss=['binary_crossentropy',lambda y_true,y_pred: y_pred],loss_weights=[1.,0.2],metrics={'sigmoid':'accuracy'})
    
    model_predict=Model([x_in,x_in_un],p)
    model_predict.compile(optimizer='adam', loss='binary_crossentropy', metrics=['accuracy'])
...
if __name__ == "__main__":
    ...
    evaluator = Evaluate()
    history=model[0].fit([x_train,x_train_un,y_train],[y_train,y_train],verbose=2,epochs=50,batch_size=batch_size,callbacks=[evaluator]) 
...
Relying on driver to perform ptx compilation. This message will be only logged once.
 - 295s - loss: 0.4601 - sigmoid_loss: 0.3940 - l2_loss_loss: 0.3303 - sigmoid_acc: 0.8200
15000/15000 [==============================] - 169s 11ms/step
acc: 0.8734666666666666 loss: 0.3069677462418874  highest: 0.8734666666666666
Epoch 2/50
 - 286s - loss: 0.3625 - sigmoid_loss: 0.2873 - l2_loss_loss: 0.3761 - sigmoid_acc: 0.8789
15000/15000 [==============================] - 164s 11ms/step
acc: 0.8840666666666667 loss: 0.28368691403071084  highest: 0.8840666666666667
Epoch 3/50
 - 276s - loss: 0.3175 - sigmoid_loss: 0.2370 - l2_loss_loss: 0.4023 - sigmoid_acc: 0.9047
15000/15000 [==============================] - 176s 12ms/step
acc: 0.8985333333333333 loss: 0.2585595059712728  highest: 0.8985333333333333
Epoch 4/50
 - 279s - loss: 0.2719 - sigmoid_loss: 0.1872 - l2_loss_loss: 0.4234 - sigmoid_acc: 0.9265
15000/15000 [==============================] - 160s 11ms/step
acc: 0.8992666666984558 loss: 0.25226998875935874  highest: 0.8992666666984558
Epoch 5/50
 - 268s - loss: 0.2275 - sigmoid_loss: 0.1382 - l2_loss_loss: 0.4465 - sigmoid_acc: 0.9473
15000/15000 [==============================] - 171s 11ms/step
acc: 0.8892666666348775 loss: 0.27950657562414805  highest: 0.8992666666984558
"""